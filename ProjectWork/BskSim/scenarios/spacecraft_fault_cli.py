...
- Configure and run spacecraft simulations with RW friction faults
- Export binary files for Vizard visualization
- Define target locations for observation

Usage:
    python spacecraft_fault_cli.py [options]
"""
import os
import sys
import inspect
...
try:
    from Basilisk.utilities import macros, vizSupport, orbitalMotion
    # Import the RW Fault Scenario
    from RW_Fault_Scenario import RWFaultScenario, run as run_scenario
except ImportError as e:
    print(f"ERROR: Could not import required modules: {e}")
    print("Make sure Basilisk and the RW_Fault_Scenario.py are in your Python path.")
    sys.exit(1)
    ...
    def customize_scenario(scenario):
        ...
        if vizSupport.vizFound and config.save_binary:
            ...
            viz = vizSupport.enableUnityVisualization(
                ...
                liveStream=False,  # Don't stream when saving binary
                saveFile=config.binary_filename
            )
    ...
    try:
        ...
        with open(f"{output_dir}/simulation_summary.txt", "w") as f:
            ...
            if config.save_binary and viz:
                binary_path = f"{config.binary_filename}.bin"
