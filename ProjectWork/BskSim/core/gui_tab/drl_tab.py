...
3. Check Results tab for output files and visualizations
    ...
    def _build_integration(self, root):
        ...
        ttk.Label(root, text="Integrated Fault Detection + DRL Task Reassignment",
                  font=("TkDefaultFont", 12, "bold")).pack(anchor="w", **pad)
        ...
        ttk.Checkbutton(config_frame, text="Enable Fault Detection", 
                       variable=tk.BooleanVar(value=True), state="disabled").pack(anchor="w")
        ttk.Checkbutton(config_frame, text="Enable DRL Task Reassignment", 
                       variable=self.integration_mode).pack(anchor="w")
        
        # Fault injection info
        fault_frame = ttk.LabelFrame(root, text="Simulation Configuration", padding=10)
        fault_frame.pack(fill="x", **pad)
        
        ttk.Label(fault_frame, text="Faults and configuration will be loaded from the main GUI tabs.",
                 wraplength=500).pack(anchor="w")
        ttk.Button(fault_frame, text="Go to Fault Configuration Tab", 
                  command=self._switch_to_fault_tab).pack(anchor="w", pady=5)

        # Run integration
        run_frame = ttk.Frame(root)
        run_frame.pack(fill="x", **pad)
        
        self.integrate_btn = ttk.Button(run_frame, text="Run Integrated System", 
                                       command=self._run_integration)
        self.integrate_btn.pack(side="left", padx=(0, 5))
        
        ttk.Button(run_frame, text="Load Previous Results", 
                  command=self._load_integration_results).pack(side="left")
    ...
    def _run_integration(self):
        ...
        messagebox.showinfo("DRL Integration", 
