...
def generate_fault_plots(fault_type, fault_data, time_data, fault_time_min, spacecraft_name="Spacecraft"):
    ...
    FIXED: Better error handling for run_scenario return values
    """
    plots = {}
    ...
    if isinstance(fault_data, dict) and fault_data.get('use_real_simulation', False) and FAULT_LOADER_AVAILABLE:
        print(f"DEBUG: Running real {fault_type} simulation...")
        ...
        try:
            ...
            print(f"DEBUG: Calling run_scenario for {fault_type}...")
            result = run_scenario(
                fault_type,
            ...
            # FIXED: Better handling of run_scenario return values
            if result is None:
                print(f"DEBUG: run_scenario returned None for {fault_type}")
                ...
                if len(result) == 3:
                    ...
                    print(f"DEBUG: Unexpected tuple length from run_scenario: {len(result)}")
                    scenario, viz, figure_list = result[0] if len(result) > 0 else None, None, {}
            else:
                # If result is not a tuple, treat it as a scenario object
                print(f"DEBUG: run_scenario returned non-tuple: {type(result)}")
                scenario = result
                viz = None
                figure_list = {}
            
            print(f"DEBUG: Processed run_scenario result - scenario: {scenario is not None}, "
                  f"viz: {viz is not None}, figure_list: {len(figure_list) if figure_list else 0}")
            
            if scenario is not None:
                print(f"DEBUG: Successfully ran {fault_type} simulation")
                ...
                # When using plots from real simulation
                ...
                # Check if scenario has a pull_outputs method
                    ...
                    try:
                        print(f"DEBUG: Attempting to extract plots using pull_outputs...")
                        scenario_plots = scenario.pull_outputs(showPlots=False)
                        if scenario_plots and len(scenario_plots) > 0:
                            print(f"DEBUG: Extracted {len(scenario_plots)} plots using pull_outputs")
                            ...
                            print(f"DEBUG: pull_outputs returned empty or None")
                    except Exception as e:
                        print(f"DEBUG: pull_outputs failed: {e}")
                
                # If no plots available from scenario, generate from extracted data
                print(f"DEBUG: Generating plots from extracted real data")
                ...
                if real_plots and len(real_plots) > 0:
                    print(f"DEBUG: Generated {len(real_plots)} plots from real data")
                    return real_plots
                else:
                    print(f"DEBUG: No plots generated from real data, falling back to synthetic")
            else:
                print(f"DEBUG: Failed to run {fault_type} simulation (scenario is None), falling back to synthetic")
                
        except Exception as e:
            print(f"ERROR: Failed to run real simulation for {fault_type}: {e}")
            import traceback
            traceback.print_exc()
            print(f"DEBUG: Falling back to synthetic plots...")
    
    # Check if fault_data is actually a scenario object with pull_outputs method
    if hasattr(fault_data, 'pull_outputs'):
        try:
            print(f"DEBUG: Extracting plots from existing {fault_type} scenario...")
    ...
    print(f"DEBUG: Using synthetic plotting for {fault_type}")
    try:
...
def generate_plots_from_real_data(fault_type, real_fault_data, scenario, fault_time_min, spacecraft_name):
    ...
    print(f"DEBUG: Generating plots from real {fault_type} fault data")
    print(f"DEBUG: Available data keys: {list(real_fault_data.keys()) if real_fault_data else 'None'}")
...
def generate_friction_plots(fault_data, time_data, fault_time_min, spacecraft_name):
    """Generate power limit fault plots - placeholder"""
    plots = {}
    print(f"DEBUG: Power limit plotting not yet implemented, using generic")
    return generate_generic_fault_plots(fault_data, time_data, fault_time_min, spacecraft_name)
...
def generate_encoder_plots(fault_data, time_data, fault_time_min, spacecraft_name):
    """Generate encoder fault plots - placeholder"""
    plots = {}
    print(f"DEBUG: Encoder plotting not yet implemented, using generic")
    return generate_generic_fault_plots(fault_data, time_data, fault_time_min, spacecraft_name)


def generate_battery_plots(fault_data, time_data, fault_time_min, spacecraft_name):
    """Generate battery fault plots - placeholder"""
    plots = {}
    print(f"DEBUG: Battery plotting not yet implemented, using generic")
    return generate_generic_fault_plots(fault_data, time_data, fault_time_min, spacecraft_name)
