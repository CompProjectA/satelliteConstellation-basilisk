...
def run(show_plots=True):
        ...
        if(i>=0 and i<4):
            ...
            if i == 0:
                ...
                if(i == 1):
                    ...
                    solarPanel.stateInMsg.subscribeTo(sat.sc.scStateOutMsg)
                    scSim.AddModelToTask(simTaskName, leaders[0].children[0].battery)
   ...
   # Communicatio between leaders
    ...
    # Battery fault integration, choosing sat 2 (of leader sat 1) to be faulty
    
    scSim.batteryReader = leaders[0].children[0].batteryReader
    ...
    leaders[0].children[0].battery.addPowerNodeToModel(powerSink.nodePowerOutMsg)
    scSim.powerSink = powerSink
    scSim.createNewEvent(
        "powerSinkFault",        
        1,               # how often to check
        True,                   
        [f"self.TotalSim.CurrentNanos >= {macros.sec2nano(120.0)}"],   # condition
        [
        # start drawing 10 W 
        "self.powerSink.nodePowerOut = -0.01",
        # disable this event so it only fires once
        "self.setEventActivity('powerSinkFault', False)"
        ]
    )
    leaders[0].children[0].battery.addPowerNodeToModel(solarPanel.nodePowerOutMsg)  
    ...
    batteryInMsg.subscribeTo( leaders[0].children[0].battery.batPowerOutMsg)
    ...
    for i in range(1,16):
        batteryPanel1 = vizSupport.vizInterface.GenericStorage()
        batteryPanel1.ModelTag = "batteryPanel"
        batteryPanel1.showName = True
        batteryPanel1.minValue = 0
        batteryPanel1.maxValue = 100
        batteryPanel1.useStorageLevel = True
        batteryInMsg = messaging.PowerStorageStatusMsgReader()
        batteryInMsg.subscribeTo( leaders[0].children[0].battery.batPowerOutMsg)
        batteryPanel1.batteryStateInMsg = batteryInMsg
        batteryPanel1.this.disown()

        batteryPanel1.thresholds = vizSupport.vizInterface.IntVector([20, 50, 80])

        batteryPanel1.color = vizSupport.vizInterface.IntVector(
            vizSupport.toRGBA255("red") +
            vizSupport.toRGBA255("orange") +
            vizSupport.toRGBA255("yellow") +
            vizSupport.toRGBA255("green")
        )
        gsList.append([batteryPanel1])
    samplingTime = unitTestSupport.samplingTime(simulationTime, simulationTimeStep, numDataPoints)
    dataLog = leaders[0].children[0].sc.scStateOutMsg.recorder(samplingTime)
    scSim.AddModelToTask(simTaskName, dataLog)

    # Battery state logger
    batteryLog = leaders[0].children[0].battery.batPowerOutMsg.recorder(samplingTime)
    ...
    if vizSupport.vizFound:
        ...
        vizSupport.setInstrumentGuiSetting(viz, 
                                            spacecraftName=leaders[0].children[0].model_tag,
                                            showGenericStoragePanel=True)
        # # Add a rotating frame for visuali
    ...
    if show_plots:
        ...
        plt.xlabel('Time (minutes)')
        plt.ylabel('Has Access (1 = Yes, 0 = No)')
        plt.title('Access Between Leader (Sat9) and Children (Sat1,5,and 13)')
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.show()
